...
class LaboratoryControllerTest {
    ...
    void findAll() {
        ...
        ResponseEntity<List<LaboratoryResponse>> response = controller.findAll();
        assertThat(response).isNotNull();
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).hasSize(1);
        assertThat(response.getBody()).isInstanceOf(List.class);
        assertThat(response.getBody().get(0)).isInstanceOf(LaboratoryResponse.class);
        assertThat(response.getBody().get(0).id()).isEqualTo(FactoryLaboratoryTestUtil.ID_TEST);
        assertThat(response.getBody().get(0).name()).isEqualTo(FactoryLaboratoryTestUtil.NAME_TEST);
    }
}